...
支持单个技能、指定目录中的全部技能，以及跨目录的已安装技能审查；不会默认扫描整台机器。全局审查应先盘点用户级、项目级、系统级和已启用插件的技能位置，再明确传入路径。缓存或已禁用插件不等于当前生效。目录软链接会被遍历，并按真实路径去重、避免循环；跳过项会写入报告。整体审查还会分析误触发、职责冲突和合理协作。
...
在获得授权且有历史记录时，可按[用量证据](references/usage-evidence.md)区分显式选择、读取说明、执行工作流和成功完成。按有观测证据的任务数排序，注明覆盖范围，排除审阅噪声及分叉历史重复。零观测不代表从未使用；原始次数也不能反映安装时长的差异。

这是供智能体使用的审阅方法，不是内置遥测采集器；静态扫描器不统计用量。结合独有价值、功能重叠和依赖关系作出取舍。用户要求时，可将专用工作流改为显式调用，或备份后移出冗余技能并记录恢复方式；不因低使用量自动删除。个人历史及用量报告不应上传公共仓库。

> 使用 $auditing-skills 精简我的个人技能组。将现有历史作为辅助证据，保留有用能力，专用工作流改为显式调用，冗余技能先备份再移出。
